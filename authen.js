function login(){
	...
	fetch("http://localhost:3000/db_login", {
		method: "POST",
		headers: {
			"Content-Type": "application/json" // sent in the body of an HTTP request or response as JSON format.
			},
...
function authen() {
    ...
    fetch('http://localhost:3000/db_authen', {
        ...
        .then(response => response.json())
        .then(data_authen => {
            if (data_authen.message == 'Token is valid') { // if got the message from router /db_authen in task2_db.js
                //pass
            } else {
